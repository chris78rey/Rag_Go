...
func (h *Handler) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	var planID int
	err = h.db.QueryRowContext(r.Context(), "SELECT id FROM plans WHERE code = $1", planCode).Scan(&planID)
	if err != nil {
...
func (h *Handler) ListUsers(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.QueryContext(r.Context(),
		`SELECT u.id, u.email, u.full_name, u.role, p.code, u.active, u.created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.Email, &u.FullName, &u.Role, &u.PlanCode, &u.Active, &createdAt); err != nil {
			slog.Error("escaneando_usuario", "error", err)
			continue
		}
		u.CreatedAt = createdAt.Format("2006-01-02T15:04:05Z07:00")
...
func (h *Handler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if req.FullName != nil {
		_, err := h.db.ExecContext(r.Context(),
			"UPDATE users SET full_name = $1 WHERE id = $2", *req.FullName, userID)
		if err != nil {
	...
	if req.Active != nil {
		_, err := h.db.ExecContext(r.Context(),
			"UPDATE users SET active = $1 WHERE id = $2", *req.Active, userID)
		if err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "actualizando usuario"})
			return
		}
...
func (h *Handler) UpdatePlan(w http.ResponseWriter, r *http.Request) {
	...
	var planID int
	err := h.db.QueryRowContext(r.Context(), "SELECT id FROM plans WHERE code = $1", req.PlanCode).Scan(&planID)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "plan no encontrado"})
		return
	}
...
func (h *Handler) GetUsage(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.UserID, &u.Email, &usageDate, &u.Count); err != nil {
			continue
		}
